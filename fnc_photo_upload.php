...
/* function resize_photo($temp_photo, $normal_photo_max_w, $normal_photo_max_h) {
	...
	$temp_image = imagecreatetruecolor($new_w, $new_h);
	// mis image objektile, kust võtame, kust koordinaatidest alustame, kust võtta x ja y, kui laialt, kui kõrgelt, kui laia võtame, kui kõrge võtame
...
function store_profile_picture($file_name){
	$notice = null;
	$conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
	$conn->set_charset("utf8");
