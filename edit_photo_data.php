<?php
	...
	$photo_error = null;
	$photo_delete_error = null;
	$alt = null;
	$privacy = 1;
	//$id = null; määran selle asemel id hoopis läbi post meetodi

	if($_SERVER["REQUEST_METHOD"] == "POST"){
		$id = $_POST["photo_input"]; //selliselt panen kirja muutuja id
		...
		if(isset($_POST["photo_delete_submit"])){
			if(isset($_POST["photo_input"]) and filter_var($_POST["photo_input"], FILTER_VALIDATE_INT)){
				$photo_delete_error = delete_photo($id);
				if(empty($photo_delete_error)){
					$photo_delete_error = "Pilt kustutatud!";
				} else {
					$photo_delete_error = "Pildi kustutamine ei õnnestunud!";
				}
			} else {
				$photo_delete_error = "Pilti ei õnnestu kustutada!";
			}
		}
	}//if POST
	...
	/*if(isset($_POST["photo_submit"])){
		if(isset($_POST["photo_input"]) and !empty($_POST["photo_input"])) {
...
<ul>
	...
	<li>Tagasi oma <a href="<?php echo "gallery_own.php?page=" .$_SESSION["gallery_own_page"]; ?>">fotode galeriisse</a></li>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ."?id=" .$id;?>">
		...
		<span><?php echo $photo_delete_error; ?></span>
	</form>
